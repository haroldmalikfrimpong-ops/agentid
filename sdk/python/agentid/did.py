...
def resolve_did_multibase(did_string: str) -> bytes:
    """Resolve any DID that uses z-prefix multibase encoding (e.g., did:agentnexus).

    Handles both raw 32-byte keys and Ed25519 multicodec-prefixed (0xed01) keys.
    """
    parts = did_string.split(":", 2)
    if len(parts) < 3:
        raise ValueError(f"Invalid DID format: {did_string!r}")

    identifier = parts[2]
    if not identifier.startswith("z"):
        raise ValueError(
            f"DID identifier does not use z-prefix multibase: {did_string!r}"
        )

    try:
        raw = _b58decode(identifier[1:])
    except ValueError as exc:
        raise ValueError(f"Failed to decode base58btc from DID: {exc}") from exc
    ...
    # Raw 32-byte key (no multicodec prefix, like did:aps)
    if len(raw) == 32:
        return raw

    raise ValueError(
        f"Cannot extract Ed25519 key from {did_string!r}. "
        f"Decoded {len(raw)} bytes — expected 32 (raw) or 34 (multicodec)."
    )
...
def resolve_did(did_string: str, did_document: Optional[dict] = None) -> bytes:
    ...
    Supports: did:agentid, did:aps, did:key, did:web, did:agentnexus,
    and any DID method using z-prefix multibase encoding.
    """
    if did_string.startswith("did:agentid:"):
        return resolve_did_agentid(did_string)
    elif did_string.startswith("did:aps:"):
        # Use multibase resolver — handles both raw 32-byte and multicodec-prefixed keys
        return resolve_did_multibase(did_string)
        ...
        # Try multibase resolution for unknown methods (did:agentnexus, etc.)
        parts = did_string.split(":", 2)
        ...
        raise ValueError(
            ...
            "or any method using z-prefix multibase encoding."
        )
